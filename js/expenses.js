...
async function loadExpenses() {
  ...
  try {
    const snap = await getDocs(collection(db, "users", CURRENT_USER.uid, "expenses"));
...
function render() {
  ...
  if (cat !== "all") records = records.filter(r => r.category === cat);
  ...
  listEl.innerHTML = records.map(r => `
    <tr>
      <td data-label="Category"><span class="badge badge-expense">${escapeHtml(r.category)}</span></td>
      <td data-label="Amount" class="cell-amount expense">${formatCurrency(r.amount)}</td>
...
[searchInput, sortSelect, categoryFilter].forEach(el => el.addEventListener("input", render));
...
document.getElementById("open-add-expense").addEventListener("click", () => openModal(null));
...
function validateForm() {
  ...
  if (!fieldEl("f-date").value) { fieldEl("f-date").classList.add("field-error"); valid = false; }
  if (!fieldEl("f-category").value) { fieldEl("f-category").classList.add("field-error"); valid = false; }
  if (!fieldEl("f-reason").value.trim()) { fieldEl("f-reason").classList.add("field-error"); valid = false; }
  return valid;
}

form.addEventListener("submit", async (e) => {
  e.preventDefault();
  if (!validateForm()) { showToast("Expense reason and all required fields must be filled.", "error"); return; }

  submitBtn.disabled = true;
  submitBtn.querySelector(".spinner")?.classList.add("active");

  const payload = {
    category: fieldEl("f-category").value,
    amount: Number(fieldEl("f-amount").value),
    reason: fieldEl("f-reason").value.trim(),
  ...
  try {
    if (EDIT_ID) {
      ...
      await updateDoc(doc(db, "users", CURRENT_USER.uid, "expenses", EDIT_ID), payload);
      await writeAuditLog({ action: "UPDATE", targetUser: CURRENT_USER.uid, targetRecord: EDIT_ID, recordType: "expenses", oldValue: before, newValue: payload, reason: "Edited via Expenses module" });
      showToast("Expense updated successfully.", "success");
    } else {
      const ref = await addDoc(collection(db, "users", CURRENT_USER.uid, "expenses"), {
        ...payload, createdAt: serverTimestamp(), createdBy: CURRENT_USER.uid, userId: CURRENT_USER.uid
      });
      await writeTransactionRecord(CURRENT_USER.uid, {
        type: "EXPENSE", amount: payload.amount, category: payload.category, reason: payload.reason,
        date: payload.date, time: payload.time, sourceCollection: "expenses", sourceId: ref.id
      });
      await writeAuditLog({ action: "CREATE", targetUser: CURRENT_USER.uid, targetRecord: ref.id, recordType: "expenses", newValue: payload });
...
async function handleDelete(id) {
  if (!confirmAction("Delete this expense record? This cannot be undone.")) return;
  try {
    const before = ALL_RECORDS.find(r => r.id === id);
    await deleteDoc(doc(db, "users", CURRENT_USER.uid, "expenses", id));
    await writeAuditLog({ action: "DELETE", targetUser: CURRENT_USER.uid, targetRecord: id, recordType: "expenses", oldValue: before });
